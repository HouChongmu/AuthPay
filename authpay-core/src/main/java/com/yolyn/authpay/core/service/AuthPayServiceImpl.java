...
import com.yolyn.authpay.db.dao.PayOrderDao;
...
public class AuthPayServiceImpl implements IAuthPayService {
    ...
    private Logger logger = LoggerFactory.getLogger(AuthPayServiceImpl.class);
...
//        checkSign();
    ...
    public ResultModel<PayConfirmResp> payConfirm(PayConfirmReq confirm) {
        ...
        try {
            ...
            if (!isCorrected) {
                throw new CommonException(CommonRespEnum.VERIFYCODE_ERROR);
            }
            //创单
    ...
    private PayOrder createPayOrder(PayApplyReq payApplyReq) {
        ...
        payOrder.setOpponentAcctNo(payApplyReq.getOpponentAcctNo());
    ...
    private void checkUser(PayApplyReq request) {
        User user = userDao.selectByUserName(request.getUserIdNo());
    ...
    private boolean checkAuthRecords(PayApplyReq request) {
        UserAuth userAuth = userAuthDao.selectByCardNo(request.getUserIdNo(), request.getUserAcctNo());
        if (null != userAuth && AuthStatusEnums.AUTHED.getCode().equals(userAuth.getAuthStatus())) {
            return true;
        } else {
            return false;
        }
    ...
    private ResultModel<PayApplyResp> payApply(PayApplyReq request) {
        try {
            ...
            //缓存
            Properties properties = PropertiesLoaderUtils.loadProperties(new ClassPathResource("core.properties"));
            Jedis jedis = new Jedis(properties.getProperty("core.redis.host"));
            SetParams setParams = new SetParams();
            setParams.ex(Integer.parseInt(properties.getProperty("core.redis.verifycode.expiretime", String.valueOf(60))));
